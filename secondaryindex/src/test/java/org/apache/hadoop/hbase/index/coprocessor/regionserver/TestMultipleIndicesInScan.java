...
public class TestMultipleIndicesInScan {
  ...
  public void testWhenThereIsNoDataInIndexRegion() throws Exception {
    ...
    Assert.assertTrue("Index flow should be used.", IndexRegionObserver.getIndexedFlowUsed());
    Assert.assertFalse("Index should fetch some seek points.",
      IndexRegionObserver.getSeekpointAdded());
  }
  ...
  public void testMultipleScansOnTheIndexRegion() throws Exception {
    ...
    Assert.assertTrue("Seekpoints should get added by index scanner",
      IndexRegionObserver.getSeekpointAdded());
    Assert.assertEquals("It should get two seek points from index scanner.", 4, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertTrue("Overall result should have only 2 rows", testRes.size() == 2);
  }
  ...
  public void testSingleLevelRangeScanForOR() throws IOException, KeeperException,
    ...
    Assert.assertTrue("Seekpoints should get added by index scanner",
      IndexRegionObserver.getSeekpointAdded());
    Assert.assertEquals("It should get two seek points from index scanner.", 6, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertTrue("Overall result should have only 6 rows", testRes.size() == 6);
  }
  ...
  public void testEqualAndRangeCombinationWithMultipleIndices() throws IOException,
    ...
    Assert.assertEquals("It should get two seek points from index scanner.", 1, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertTrue("Overall result should have only 2 rows", testRes.size() == 1);
  }
  ...
  public void testEqualAndRangeCombinationWithMultipleIndicesPart2() throws IOException,
    ...
    Assert.assertEquals("It should get two seek points from index scanner.", 4, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertTrue("Overall result should have only 2 rows", testRes.size() == 4);
  }
  ...
  public void testIfAllScannersAreRangeInAllLevels() throws IOException, KeeperException,
    ...
    Assert.assertEquals("It should get two seek points from index scanner.", 6, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertTrue("Overall result should have only 2 rows", testRes.size() == 6);
  }
  ...
  public void testANDWithORbranchesWhereEachBranchHavingAtleastOneFilterOtherThanSCVF()
    ...
    System.out.println("************* result count......***********   " + testRes.size()
        + " ###### " + testRes);
    Assert.assertFalse("Index flow should get used.", IndexRegionObserver.getIndexedFlowUsed());
  }
  ...
  public void testIndexScanWithCaching() throws Exception {
    ...
    Assert.assertEquals("Index should fetch 7 seek points", 7, IndexRegionObserver
        .getMultipleSeekPoints().size());
    Assert.assertEquals("Final result should have 7 rows.", 7, testRes.size());
    Assert.assertEquals("All rows should be fetched in single next call.", 2, nextCalled);
  }
